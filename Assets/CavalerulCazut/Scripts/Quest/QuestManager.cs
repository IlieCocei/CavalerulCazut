...
namespace CavalerulCazut
{
    ...
    {
        public Quest[] quests;
        public GameObject mesajMisiune;
        ...
        {
            using StreamReader reader = new StreamReader("Assets/CavalerulCazut/DB/QuestDB.json");
                ...
                if (quest.status == QuestStatus.ACTIVE)
                {
                        ...
                        (targetUid) => sender.GetComponent<UniqueId>().Uid == targetUid))
                    {
                        ...
                        if (quest.amount == 0)
                        {
                            ...
                            mesajMisiune.SetActive(true);
                            StartCoroutine(DezactiveazaMesajulDupa(2f));
                        }
                    }
                }
            }
        ...
        private IEnumerator DezactiveazaMesajulDupa(float delay)
        {
            yield return new WaitForSeconds(delay);
            mesajMisiune.SetActive(false);
        }
    }
}
